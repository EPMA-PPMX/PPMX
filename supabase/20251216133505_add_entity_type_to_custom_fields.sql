/*
  ...
  1. Changes
    - Add `entity_type` column to custom_fields table to distinguish between different entity types
...
ADD COLUMN IF NOT EXISTS entity_type text NOT NULL DEFAULT 'project' CHECK (entity_type IN ('project', 'resource', 'task', 'risk', 'issue', 'change_request', 'priority', 'initiation_request', 'status_report', 'action_item'));
